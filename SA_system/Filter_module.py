...
class Filter_module():
	...
	def __init__(self, data):
		...
		self.logger = Logger(__name__).get_log()
		self.db = MongoClient().packet_flow.event_log
			...
			if self.data["command"] in interfaces:
				for discription, pattern in  SUSPICIOUS_REQUEST_PATTERN:
					if re.search(pattern, repr(self.data["message"]), re.I):
						...
						self.logger.critical("detect some attack event: "+discription + str(self.data))
						self.event_logger.critical("detect some attack event: "+discription + str(self.data))
