...
def nmap_detect(data):
    ...
    for key in frequency_data:
        ...
        print frequency_data[key]
        print key
    
    X.loc[:,['Attack Length']] = 0
    if frequency_data[max_key] * 1.0 / (port_num_sum * 1.0) > 0.6 and row_num < 1000 and frequency_data[max_key] > 10:
        X.loc[(X['Source IP'] == max_key[1]) & (X['Dest IP'] == max_key[0]), ['Attack Length']] = 1
